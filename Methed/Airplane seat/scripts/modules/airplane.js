...
const createBlockSeat = (n, count) => {
	...
	// i - номер ряда
	// wrapperRow - обертка ряда, seats - ряд кресел, seat -кресло
	// C помощью map перебираем массив с буквами ABCDEF 
...
//принимаем main и data
const airplane = (main, data) => {
	const title = 'Выберите места';
	//план самолета: выходы и ряды с пассажирами
	const scheme = ['exit', 11, 'exit', 1, 'exit', 17, 'exit'];

	//в main передаем. создаем самолет, в него передаем title и scheme
